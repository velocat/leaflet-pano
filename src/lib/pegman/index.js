...
export const Pegman = L.Control.Pegman = L.Control.extend({
	...
	onAdd: function(map) {
		...
		this._panoDiv = L.DomUtil.create('div', 'pano-canvas', this._viewDiv);
		this._panoDiv.style.display = 'none';
		this._panoDiv.id = 'pano-div';
		this.options.apiKey = config.google ? config.google : this.options.apiKey;
		...
		this.Utils.loadScripts.bind(this, true).call();
		
		//this._container.addEventListener('touchstart', this.Utils.loadScripts.bind(this, !L.Browser.touch), { once: true });
		//this._container.addEventListener('mousedown', this.Utils.loadScripts.bind(this, true), { once: true });
	...
	onMapClick: function(e) {
		if (this._streetViewLayerEnabled && this.options.mapclick)
	...
	onPanoramaChangeView: function() {
		...
		let iconPos = this.Utils.setIconPosition(h);
